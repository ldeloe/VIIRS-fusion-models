...
class WNet_Separate_Decoders_Uncertainty(torch.nn.Module):
    ...
    def __init__(self, options):
        ...
        # decoding blocks
        self.expand_blocks_sic = torch.nn.ModuleList()
        ...
        for expand_n in range(len(options['unet_conv_filters']), 0, -1): #range(star,t stop, step)
            ...
            if expand_n == len(options['unet_conv_filters']):
                self.expand_blocks_sic.append(ExpandingBlock(options=options,
        ...
        # decoding blocks
        self.expand_blocks_sod = torch.nn.ModuleList()
        # Note: this structure differs from unet.py, which wouldn't accurately sum the input channels from
        # the skip connections
        for expand_n in range(len(options['unet_conv_filters']), 0, -1): #range(star,t stop, step)

            if expand_n == len(options['unet_conv_filters']):
                self.expand_blocks_sod.append(ExpandingBlock(options=options,
                                                     input_n=options['unet_conv_filters'][expand_n-1]*3,
                                                     output_n=options['unet_conv_filters'][expand_n - 1]))
            else:
                self.expand_blocks_sod.append(ExpandingBlock(options=options,
                                                     ...
                                                     output_n=options['unet_conv_filters'][expand_n - 1]))   
         # decoding blocks
        self.expand_blocks_floe = torch.nn.ModuleList()
        # Note: this structure differs from unet.py, which wouldn't accurately sum the input channels from
        # the skip connections
        for expand_n in range(len(options['unet_conv_filters']), 0, -1): #range(star,t stop, step)

            if expand_n == len(options['unet_conv_filters']):
                self.expand_blocks_floe.append(ExpandingBlock(options=options,
                                                     input_n=options['unet_conv_filters'][expand_n-1]*3,
                                                     output_n=options['unet_conv_filters'][expand_n - 1]))
            else:
                self.expand_blocks_floe.append(ExpandingBlock(options=options,
    ...
    def forward(self, x):
        ...
        for expand_block_sic in self.expand_blocks_sic:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_sic = expand_block_sic(x_expand_sic, x_contract_sv[up_idx - 1],x_contract_aea[up_idx - 1])
            up_idx -= 1

        up_idx = len(x_contract)
        x_expand_sod = x_expand
        for expand_block_sod in self.expand_blocks_sod:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_sod = expand_block_sod(x_expand_sod, x_contract_sv[up_idx - 1],x_contract_aea[up_idx - 1])
            up_idx -= 1

        up_idx = len(x_contract)
        x_expand_floe = x_expand
        for expand_block_floe in self.expand_blocks_floe:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_floe = expand_block_floe(x_expand_floe, x_contract_sv[up_idx - 1],x_contract_aea[up_idx - 1])
            up_idx -= 1

        # Predict mean and log-variance for SIC
        sic_outputs = self.regression_layer(x_expand_sic.permute(0, 2, 3, 1))
