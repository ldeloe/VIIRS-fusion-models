...
class WNet_Separate_VIIRS_Decoders(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for expand_block_sic in self.expand_blocks_sic:
            ...
            x_expand_sic = expand_block_sic(x_expand_sic, x_contract_v[up_idx - 1],x_contract_arc[up_idx - 1])
            up_idx -= 1
        ...
        for expand_block_sod in self.expand_blocks_sod:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_sod = expand_block_sod(x_expand_sod, x_contract_v[up_idx - 1],x_contract_arc[up_idx - 1])
            up_idx -= 1
        ...
        for expand_block_floe in self.expand_blocks_floe:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_floe = expand_block_floe(x_expand_floe, x_contract_v[up_idx - 1],x_contract_arc[up_idx - 1])
            up_idx -= 1
