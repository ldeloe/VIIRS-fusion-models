...
class WNet_Separate_VIIRS_Decoders(torch.nn.Module):
    ...
    def __init__(self, options):
        ...
        # decoding blocks
        self.expand_blocks_sic_acc = torch.nn.ModuleList()
        ...
        for expand_n in range(len(options['unet_conv_filters']), 0, -1): #range(star,t stop, step)
            ...
            if expand_n == len(options['unet_conv_filters']):
                self.expand_blocks_sic_acc.append(ExpandingBlock(options=options,
        ...
        self.sic_acc_feature_map = FeatureMap(input_n=options['unet_conv_filters'][0], output_n=3)
    ...
    def forward(self, x):
        ...
        x_expand_sic_acc = x_expand
        for expand_block_sic_acc in self.expand_blocks_sic_acc:
            # pass the current x, as well as the corresponding encoder layers
            x_expand_sic_acc = expand_block_sic_acc(x_expand_sic_acc, x_contract_v[up_idx - 1],x_contract_arc[up_idx - 1])
            up_idx -= 1
        ...
        return {'SIC': self.regression_layer(x_expand.permute(0, 2, 3, 1)),
                ...
                'SIC-ACC': self.sic_acc_feature_map(x_expand_sic_acc),}
