...
class TrainValDataset(Dataset):
    ...
    def random_crop(self, scene, viirs_ist): #viirs_ist because it's been masked
        ...
        ### NEW ###   
        sic_check = np.sum(scene['SIC'].values[0, row_rand: row_rand + self.options['patch_size'], col_rand: col_rand + self.options['patch_size']].numpy() != self.options['class_fill_values']['SIC']) 

        sic_not_fill = np.count_nonzero(scene['SIC'].values[0, row_rand: row_rand + self.options['patch_size'], col_rand: col_rand + self.options['patch_size']].numpy() != self.options['class_fill_values']['SIC'])
        viirs_not_fill = np.count_nonzero(viirs_ist[row_rand:row_rand+self.options['patch_size'], col_rand:col_rand+self.options['patch_size']] != self.options['train_fill_value'])
        print("random_crop SIC NOT FILL: ", sic_not_fill)
        print("random_crop VIIRS NOT FILL: ", viirs_not_fill)
        #viirs_check = np.sum(viirs_ist[row_rand:row_rand+self.options['patch_size'], col_rand:col_rand+self.options['patch_size']] != self.options['train_fill_value'])

        if (viirs_not_fill > (sic_not_fill/2)) and sic_check > 1:    
    ...
    def random_crop_downsample(self, idx):
        ...
        ### NEW ###
        sic_not_fill = np.count_nonzero(self.scenes[idx][0, row_rand: row_rand + self.options['patch_size'], col_rand: col_rand + self.options['patch_size']].numpy() != self.options['class_fill_values']['SIC'])
        #viirs = self.scenes_viirs[idx]
        viirs = self.mask_viirs(self.scenes_viirs[idx])
        viirs_not_fill = np.count_nonzero(viirs[row_rand:row_rand+self.options['patch_size'], col_rand:col_rand+self.options['patch_size']] != self.options['train_fill_value'])
        print("SIC NOT FILL: ", sic_not_fill)
        print("VIIRS NOT FILL: ", viirs_not_fill)
        # use patch if more than half of the valid pixels contain ist data: if viirs_not_fill > (sic_not_fill/2)
        #ADD: have viirs_check count the number of pixels with ist data
        # count how many fill values (sic fill??). subtract the number of fill values in the viirs data. use if 50% sic fill remain
        ###UPDATE random_crop as well!!!!
        ...
        #if (viirs_not_fill > (sic_not_fill/2)) and sic_check > 1: #failed
        if viirs_not_fill > 13100 and sic_check > 1:
            patch[0:len(self.options['full_variables']), :, :] = \
    ...
    def __getitem__(self, idx):
        ...
        while sample_n < self.options['batch_size']:
            ...
            try:
                if self.downsample:
                    x_patch, y_patch = self.random_crop_downsample(scene_id) # does this use the masked ist data??
                else:
                    scene = xr.open_dataset(os.path.join(
